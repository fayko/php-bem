...
class Bem_Curl_Multi
{
     ...
     * @param array $callbackOpts
     * @param array $curlOpts
     */
    public function addTread($url, $callback, $callbackOpts = array(),  $curlOpts = array())
    {
            ...
            while($done = curl_multi_info_read($mh)) {
                ...
                foreach($this->_threads as $id => $thread){
                    ...
                    if ($thread['handler'] === $done['handle']) {
                        ...
                        $callbackOpts = !empty($thread['callback_opts']) ? array_merge($callbackOpts, $thread['callback_opts']) : $callbackOpts;
                        # Call the callback.
                        call_user_func_array($thread['callback'], $callbackOpts);
